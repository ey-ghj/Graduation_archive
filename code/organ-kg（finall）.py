...
# 创建子图，并绘制知识图谱
subgraph = G.subgraph(top_nodes).copy()  # 创建一个新的可变图形对象
isolated_nodes = [node for node in subgraph.nodes() if len(subgraph.edges(node)) == 0]
...
plt.axis('off')
plt.savefig("./output/organ_kg1.png", dpi=300)
nx.write_gexf(G, "./output/organ_graph.gexf")
plt.show()
